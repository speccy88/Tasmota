...
The HTTPS path is stock Tasmota Berry `webclient()`/BearSSL. TasmoClaw defaults to compact prompt mode with a bounded context byte limit so regular ESP32 boards without PSRAM are less likely to hit webclient `HTTP -8` ("too little RAM") during DeepSeek POST requests. Local OpenAI-compatible servers such as Ollama and MLX-LM can be reached over plain HTTP with Berry `tcpclient`. Full TasmoClaw uses direct Brave Search API calls; Lite has no web search tools.
...
- Full TasmoClaw now includes guided workflows for device health checks, Waveshare bring-up checks, current rule explanation, profile/personality memory, plain-language light timer automation, and simple display status dashboards.
