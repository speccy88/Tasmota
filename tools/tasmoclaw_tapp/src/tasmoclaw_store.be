...
class TasmoClawStore
  ...
  def read_file(file)
    try
      if path.exists(file) != true return nil end
      var f = open(file, 'r')
      var data = f.read()
      f.close()
      return data
  ...
  def load_config()
    ...
    try
      ...
      if raw != nil
        ...
        for k:obj.keys() cfg[k]=obj[k] end
      end
    except .. as e,m
      ...
      try
        ...
        if raw2 != nil
          ...
          for k:obj2.keys() cfg[k]=obj2[k] end
        end
      except .. as e2,m2
      end
    end
  ...
  def save_config(cfg)
    ...
    if !r['ok']
      try persist.set(self.config_file, tasmoclaw_util.json_encode(cfg)); return {'ok':true,'fallback':'persist','warning':r['error']} except .. as e,m end
    end
    return r
  end
  ...
  def load_history()
    try
      ...
      try
        ...
        if raw2 != nil return json.load(raw2) end
      except .. as e2,m2
      end
    end
    return []
  ...
  def save_history(h)
    ...
    if !r['ok']
      try persist.set(self.history_file, tasmoclaw_util.json_encode(h)); return {'ok':true,'fallback':'persist','warning':r['error']} except .. as e,m end
    end
    return r
  end
  ...
  def load_pending()
    try
      ...
      if raw != nil && size(raw) > 0
        var p = json.load(raw)
        if p == nil || type(p) != 'map' return nil end
        return p
      end
    except .. as e,m
  ...
  def save_pending(p)
    if p == nil
      ...
      if !r['ok']
        try persist.remove(self.pending_file); return {'ok':true,'fallback':'persist','warning':r['error']} except .. as e,m end
      end
      return r
    else
      var r2 = self.write_file(self.pending_file, tasmoclaw_util.json_encode(p))
      if !r2['ok']
        try persist.set(self.pending_file, tasmoclaw_util.json_encode(p)); return {'ok':true,'fallback':'persist','warning':r2['error']} except .. as e2,m2 end
      end
      return r2
    end
  end
  ...
  def ensure_workspace()
    self.workspace_fallback = false
    try
      if path.exists('/tasmoclaw') != true path.mkdir('/tasmoclaw') end
      if path.exists('/tasmoclaw/berry') != true path.mkdir('/tasmoclaw/berry') end
      if path.exists('/tasmoclaw/logs') != true path.mkdir('/tasmoclaw/logs') end
