...
class TasmoClawTools
  ...
  def tool_lines_compact()
    ...
    if self.skill_active('device')
      out += '- sensor_read: temperature/humidity/ADC/I2C; power_read/control; rule_control; timer_control; DisplayText/LVGL/WebColor; I2S audio; light/MQTT/telemetry/network/system\n'
    end
  ...
  def image_inspect(args)
    ...
    if image_url == nil || image_url == ''
      ...
      if p != nil && p != ''
        return {'ok':false,'error':'Stock TasmoClaw image_inspect supports image_url only. File upload/base64 from SD or FlashFS needs a host/browser helper or custom firmware bridge.','path':p}
      end
